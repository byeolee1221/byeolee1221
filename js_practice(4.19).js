...
// form, input, button, answer_box 모두 연결해야함.
// 버튼을 누르면 기능이 구현되므로 버튼에 이벤트리스너를 추가해야함.

const formElement = document.querySelector('.question_form');
const userInputElement = document.getElementById('user_answer');
const btnElement = document.getElementById('answer_btn');
const displayAnswerboxElement = document.querySelector('.answer_box');
const errorPgElement = document.querySelector('.error');

function checkAnswer() {
    const enteredAnswer = userInputElement.value;

    if (enteredAnswer === '') {
        userInputElement.classList.add('error');
        return;
    } else if (enteredAnswer === '검은사막') {
        displayAnswerboxElement.style.display = 'block';
    } else {
        displayAnswerboxElement.style.display = 'block';
        displayAnswerboxElement.textContent = '땡!! 정답을 다시 확인하세요!';
    }
}

btnElement.addEventListener('click', checkAnswer);

// 네 번째 컨테이너 (while 반복문 연습하기)

const forthUserInputElement = document.getElementById('user_roll');
const forthRollDiceBtnElement = document.getElementById('rollDice_btn');
const rollDiceListElement = document.querySelector('.roll_dice_info');
const forthUserInputSpanElement = document.getElementById('forth_user_input');
const rollDiceSpanElement = document.getElementById('dice_roll_num');

function rollDice() {
    return Math.floor(Math.random() * 6) + 1;
}

function rollDiceInfo() {
    const enteredUserNum = forthUserInputElement.value;
    let numberOfRoll = 0;
    ...
    while(!rolledTargetNum) {
        ...
        const rollDiceInfoListElement = document.createElement('li');
        const outputText = `${numberOfRoll}번째 굴린 숫자: ${rolledNum}`;
        rollDiceInfoListElement.textContent = outputText;
        rollDiceListElement.append(rollDiceInfoListElement);

        rolledTargetNum = rolledNum == enteredUserNum;
    }

    forthUserInputSpanElement.textContent = enteredUserNum;
    rollDiceSpanElement.textContent = numberOfRoll;
}

forthRollDiceBtnElement.addEventListener('click', rollDiceInfo);